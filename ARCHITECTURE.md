...
## UX Rules (Food Entry Date Selection)

**Поведение:** при добавлении еды (add_food / food_entries) пользователь может находиться на выбранном прошлом дне. В этом случае приложение спрашивает, куда записать еду.

**Определения:**
- **Сегодня** = `LocalDate.now()` в формате `yyyy-MM-dd`
- `selectedDate == null` трактуется как «сегодня» (записывать без диалога)

**Условие показа диалога:**
- Диалог показывать **только если** в ответе AI есть `food_entries` **и** выбранный день ≠ сегодня

**Варианты диалога:**
- **Выбранный день (D)** — записать еду на выбранную дату D
- **Сегодня** — записать еду на сегодня
- **Отмена** — не записывать, закрыть диалог