...
interface DayEntryRepository {
    ...
    /**
     * Получить все записи за указанную дату (suspend). Для кэширования и prefetch.
     * @param date дата в формате "yyyy-MM-dd"
     */
    suspend fun getEntriesForDate(date: String): List<DayEntry>
    ...
    /**
     * Ккал по дням за указанный период startDate..endDate inclusive.
     * Только дни с записями, без заполнения пропусков.
     */
    fun getKcalPerDayFlow(startDate: String, endDate: String): Flow<List<DayKcalPoint>>

    /**
     * Ккал по дням за последние 7 дней (endDate из getCurrentDateFlow, startDate = endDate - 6).
     * Только дни с записями, без заполнения пропусков.
     */
    fun getKcalHistoryLast7DaysFlow(): Flow<List<DayKcalPoint>>

    /**
     * Максимальная сумма ккал за один день по всей БД (для стабильной шкалы Y графика).
     */
    fun getGlobalMaxDailyKcalFlow(): Flow<Int>
}

/**
 * Точка (дата, ккал) для графика истории.
 */
data class DayKcalPoint(val date: String, val kcal: Int)
